    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/settings" element={<RequireManager><Settings /></RequireManager>} />
          ...
          {/* ⭐️ /pre-order เปิดให้ทุก role ที่ล็อกอิน (รวม staff) — พนักงานก็สั่งจองของตัวเองได้
              เดิมเป็น RequireMember (MEMBER เท่านั้น) แต่ตอนนี้ staff ที่กดเข้า LINE ก็ต้องจองสินค้า
              ของตัวเองได้เหมือนสมาชิก → ใช้ RequireAuth; backend ยังเช็ค ownership ออเดอร์แต่ละใบ */}
          <Route path="/pre-order" element={<RequireAuth><Preorder /></RequireAuth>} />
