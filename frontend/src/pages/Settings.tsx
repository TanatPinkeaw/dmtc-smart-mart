...
export default function Settings() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans p-4 md:p-6 relative pb-20 md:pb-6">
      
      {/* Header */}
      <div className="max-w-7xl mx-auto flex items-center gap-3 text-indigo-600 mb-6 bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6">
        ...
        {/* Content Area */}
        <div className="flex-1 bg-white rounded-3xl shadow-sm border border-gray-100 p-4 md:p-8 min-h-[500px] relative">
          
          {/* TAB 1: ร้านค้า */}
          {activeTab === 'STORE' && (
            <form onSubmit={handleSaveStore} className="space-y-4 animate-fade-in max-w-2xl">
              <h2 className="text-lg md:text-xl font-bold mb-4 flex items-center gap-2"><Store className="text-indigo-500"/> ข้อมูลร้านค้า</h2>
              ...
              <button type="submit" className="w-full md:w-auto bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition flex justify-center items-center gap-2 mt-4">
          ...
          {activeTab === 'HISTORY' && (
            <div className="animate-fade-in flex flex-col h-full">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
                <h2 className="text-lg md:text-xl font-bold flex items-center gap-2"><History className="text-indigo-500"/> ประวัติการขาย</h2>
                <div className="flex flex-wrap items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200 w-full lg:w-auto justify-between lg:justify-start">
                  ...
                  <button onClick={fetchSalesHistory} className="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-indigo-700 w-full mt-2 lg:mt-0 lg:w-auto text-center">ค้นหา</button>
                </div>
              </div>
                    ...
                    {salesHistory.length === 0 ? (
                      ...
                      salesHistory.map(bill => (
                        <tr key={bill.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-3 font-bold text-blue-600">฿{Number(bill.total_amount).toFixed(2)}</td>
                          ...
                          <td className="p-3 text-center flex justify-center gap-2">
                            <button onClick={() => handleViewBill(bill)} className="text-indigo-500 bg-indigo-50 hover:bg-indigo-100 p-2 rounded-lg transition"><Eye size={18}/></button>
              ...
              <div className="md:hidden flex flex-col gap-3">
                {salesHistory.length === 0 ? (
                  <div className="p-8 text-center text-gray-400 border border-dashed rounded-xl">ไม่พบข้อมูลการขาย</div>
                ) : (
                  salesHistory.map(bill => (
                    <div key={bill.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative">
                      ...
                      <p className="font-bold text-blue-600 text-lg mb-4">ยอด: ฿{Number(bill.total_amount).toFixed(2)}</p>
                      <div className="flex gap-2">
                        <button onClick={() => handleViewBill(bill)} className="flex-1 bg-indigo-50 text-indigo-600 font-bold py-2 rounded-lg text-sm flex justify-center items-center gap-1"><Eye size={16}/> ดูบิล</button>
          ...
          {activeTab === 'PRODUCTS' && (
            <div className="animate-fade-in">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
                <h2 className="text-lg md:text-xl font-bold flex items-center gap-2"><Package className="text-indigo-500"/> สินค้าในระบบ</h2>
                <button onClick={() => setActiveModal('ADD_PRODUCT')} className="w-full sm:w-auto bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-indigo-700 flex justify-center items-center gap-2 transition"><Plus size={18}/> เพิ่มสินค้า</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {products.map(p => (
                  <div key={p.id} className="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 relative group">
                    ...
                    <div className="flex justify-between items-end mt-2">
                      <span className="text-xl md:text-2xl font-bold text-blue-600">฿{Number(p.price).toFixed(2)}</span>
                      <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-md">สต๊อก: {p.stock}</span>
                    </div>

                    {/* ⭐️ ปุ่มแก้ไข (ดินสอ) และ ลบ (ถังขยะ) คู่กัน */}
                    <div className="absolute top-3 right-3 flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition bg-white p-1 rounded-lg shadow-sm border border-gray-100 md:border-0 md:shadow-none">
                      <button onClick={() => { setEditingProduct(p); setActiveModal('EDIT_PRODUCT'); }} className="text-blue-400 hover:text-blue-600 hover:bg-blue-50 p-1.5 rounded-md transition" title="แก้ไขสินค้า">
                        <Edit size={16}/>
                      </button>
          ...
          {activeTab === 'CATEGORIES' && (
            <div className="animate-fade-in">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
                <h2 className="text-lg md:text-xl font-bold flex items-center gap-2"><Tags className="text-indigo-500"/> หมวดหมู่สินค้า</h2>
                <button onClick={() => setActiveModal('ADD_CATEGORY')} className="w-full sm:w-auto bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-indigo-700 flex justify-center items-center gap-2 transition"><Plus size={18}/> เพิ่มหมวดหมู่</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                {categories.map(c => (
                  <div key={c.id} className="bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center group shadow-sm">
          ...
          {activeTab === 'SUPPLIERS' && (
            <div className="animate-fade-in">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
                <h2 className="text-lg md:text-xl font-bold flex items-center gap-2"><Truck className="text-indigo-500"/> ซัพพลายเออร์</h2>
                <button onClick={() => setActiveModal('ADD_SUPPLIER')} className="w-full sm:w-auto bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-indigo-700 flex justify-center items-center gap-2 transition"><Plus size={18}/> เพิ่มบริษัท</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {suppliers.map(s => (
                  <div key={s.id} className="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center group relative">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-500"><Truck size={20}/></div>
                      <div>
          ...
          {activeTab === 'USERS' && (
            <div className="animate-fade-in">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
                <h2 className="text-lg md:text-xl font-bold flex items-center gap-2"><Users className="text-indigo-500"/> พนักงานในระบบ</h2>
                <button onClick={() => setActiveModal('ADD_USER')} className="w-full sm:w-auto bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-indigo-700 flex justify-center items-center gap-2 transition"><Plus size={18}/> เพิ่มพนักงาน</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {users.map(u => (
                  <div key={u.id} className="bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col group relative">
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-full flex items-center justify-center font-bold text-white shadow-inner text-lg ${u.role === 'ADMIN' ? 'bg-gradient-to-br from-purple-400 to-purple-600' : 'bg-gradient-to-br from-blue-400 to-blue-600'}`}>
                    ...
                    <div className="flex justify-between items-end mt-auto">
                      <span className={`px-2 py-1 rounded-md text-[10px] md:text-xs font-bold ${u.role === 'ADMIN' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>{u.role}</span>
                    </div>
                    <button onClick={() => handleDeleteUser(u.id)} className="absolute top-3 right-3 text-red-400 hover:text-red-600 bg-white p-1 rounded-md shadow-sm border border-gray-100 md:opacity-0 md:group-hover:opacity-100 transition"><Trash2 size={16}/></button>
                  </div>
                ))}
              </div>
            </div>
      ...
      {viewingBillItems && viewingBillInfo && (
        <CustomModal title={`บิล #${viewingBillInfo.id}`} onClose={() => { setViewingBillItems(null); setViewingBillInfo(null); }}>
          ...
          <div className="overflow-y-auto max-h-60 mb-4 border rounded-lg">
            <table className="w-full text-left text-xs md:text-sm">
              <thead className="bg-gray-50 text-gray-600 sticky top-0"><tr><th className="p-2 border-b">สินค้า</th><th className="p-2 border-b text-center">จำนวน</th><th className="p-2 border-b text-right">รวม</th></tr></thead>
              <tbody>
                {viewingBillItems.map((item, idx) => (
                  <tr key={idx} className="border-b last:border-0"><td className="p-2 font-bold text-gray-800">{item.product_name}</td><td className="p-2 text-center">{item.quantity}</td><td className="p-2 text-right font-bold text-blue-600">฿{Number(item.subtotal).toFixed(2)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="flex justify-between items-center p-4 bg-indigo-50 rounded-xl mb-4 border border-indigo-100">
            <span className="font-bold text-indigo-800 text-sm">ยอดรวมทั้งสิ้น</span><span className="text-xl md:text-2xl font-bold text-indigo-600">฿{Number(viewingBillInfo.total_amount).toFixed(2)}</span>
          </div>
        ...
        <CustomModal title="แก้ไขข้อมูลสินค้า" onClose={() => { setActiveModal(null); setEditingProduct(null); }}>
          <form onSubmit={handleEditProduct} className="space-y-3 md:space-y-4">
            ...
            <div>
              ...
              <select className="w-full p-2.5 md:p-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm md:text-base" value={editingProduct.category_id || ''} onChange={e => setEditingProduct({...editingProduct, category_id: e.target.value})}>
            ...
            <button type="submit" className="w-full bg-blue-600 text-white p-3 rounded-xl font-bold hover:bg-blue-700 mt-2 transition">
              บันทึกการแก้ไข
            </button>
          </form>
        </CustomModal>
      ...
      {activeModal === 'ADD_PRODUCT' && (
        <CustomModal title="เพิ่มสินค้าใหม่" onClose={() => setActiveModal(null)}>
          <form onSubmit={handleAddProduct} className="space-y-3 md:space-y-4">
            ...
            <div>
              <label className="block text-xs md:text-sm font-bold text-gray-700 mb-1">หมวดหมู่</label>
              <select className="w-full p-2.5 md:p-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm md:text-base" value={newProduct.category_id} onChange={e => setNewProduct({...newProduct, category_id: e.target.value})}>
                <option value="">-- ไม่ระบุ --</option>
                {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white p-3 rounded-xl font-bold hover:bg-indigo-700 mt-2">บันทึกสินค้าใหม่</button>
          </form>
        </CustomModal>
      )}
        ...
        <CustomModal title="เพิ่มพนักงาน" onClose={() => setActiveModal(null)}>
          <form onSubmit={handleAddUser} className="space-y-4">
            ...
            <div>
              ...
              <select className="w-full p-2.5 md:p-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm md:text-base" value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})}>
                <option value="CASHIER">แคชเชียร์</option>
                <option value="ADMIN">ผู้จัดการ (ADMIN)</option>
              </select>
            </div>
            <button type="submit" className="w-full bg-indigo-600 text-white p-3 rounded-xl font-bold hover:bg-indigo-700 mt-2">สร้างบัญชี</button>
          </form>
        </CustomModal>
      )}
      ...
      {activeModal === 'ADD_CATEGORY' && (
        <CustomModal title="เพิ่มหมวดหมู่" onClose={() => setActiveModal(null)}>
          <form onSubmit={handleAddCategory} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white p-3 rounded-xl font-bold hover:bg-indigo-700 mt-2">เพิ่มหมวดหมู่</button>
          </form>
        </CustomModal>
      )}
      ...
      {activeModal === 'ADD_SUPPLIER' && (
        <CustomModal title="เพิ่มตัวแทนจำหน่าย" onClose={() => setActiveModal(null)}>
          <form onSubmit={handleAddSupplier} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white p-3 rounded-xl font-bold hover:bg-indigo-700 mt-2">บันทึกข้อมูล</button>
          </form>
        </CustomModal>
      )}
...
const TabButton = ({ icon, label, isActive, onClick }: any) => (
  <button onClick={onClick} className={`shrink-0 snap-start flex items-center gap-2 px-4 py-3 md:p-4 rounded-xl font-bold text-sm md:text-base transition ${isActive ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'}`}>
...
const Input = ({ label, value, onChange, type = "text", required = true, disabled = false }: any) => (
  <div>
    ...
    <input 
      ...
      className={`w-full p-2.5 md:p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition text-sm md:text-base ${disabled ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : ''}`} 
    />
  </div>
);
...
const CustomModal = ({ title, onClose, children }: any) => (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] flex items-end md:items-center justify-center sm:p-4 animate-fade-in">
    <div className="bg-white rounded-t-3xl md:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col transform transition-all">
      <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-3xl md:rounded-t-none">
