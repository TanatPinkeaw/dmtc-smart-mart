...
export default function POS() {
  ...
  const handleCheckout = async () => {
    ...
    if (!amountReceived || Number(amountReceived) < grandTotal) return Swal.fire({ icon: 'error', title: 'รับเงินมาไม่พอ!' });
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `เลขที่บิล: <b>#${response.data.receipt.sale_id}</b><br/><br/>เงินทอน: <b style="color: #db2777; font-size: 24px;">฿${response.data.receipt.change_amount}</b>`,
