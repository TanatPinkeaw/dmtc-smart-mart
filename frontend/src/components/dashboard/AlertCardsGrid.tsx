...
export function AlertCardsGrid({ lowStock, voidSummary, shiftAnomalies, openShifts, pendingApprovalShifts, onOpenDetail }: AlertCardsGridProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-5">
      {cards.map(a => (
        <div key={a.type} onClick={() => onOpenDetail(a.type, a.title)} className={`bg-white border ${a.border} rounded-3xl p-3 shadow-sm cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all duration-150 active:scale-95 focus-visible:ring-2 focus-visible:ring-brand`}>
