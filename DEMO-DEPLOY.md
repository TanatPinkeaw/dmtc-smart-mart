...
⭐️ setup key ส่งผ่าน header แล้ว (ไม่ใช่ query string) เปิดลิงก์ตรงๆ ในเบราว์เซอร์ไม่ได้ ต้องใช้ curl (แทน `<SETUP_KEY>` ด้วยค่าที่ตั้งไว้ในขั้น B):

```bash
curl -H "X-Setup-Key: <SETUP_KEY>" "https://dmtc-mart-api.onrender.com/api/create-admin"
```

จะได้บัญชี **รหัสนักศึกษา: `admin`** รหัสผ่านชั่วคราวสุ่มใหม่ทุกครั้ง — ดูใน Render service logs

จากนั้นเปิดเว็บจริง `https://dmtc-smart-mart.vercel.app` → ล็อกอินด้วย `admin` / รหัสผ่านชั่วคราวจาก log → ระบบจะบังคับเปลี่ยนรหัสผ่านก่อนใช้งานหน้าอื่น
