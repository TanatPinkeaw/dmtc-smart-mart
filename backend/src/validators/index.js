// ═══════════════════════════════════════════════════════════════════════════════════
// 📄 validators/index.js — schema ตรวจความถูกต้องของข้อมูลที่ client ส่งมา (ใช้ไลบรารี Joi)
// ─────────────────────────────────────────────────────────────────────────────────────
// ทำอะไร: นิยาม schema ของ request body แต่ละ endpoint (เช่น checkout, สร้างสินค้า, เปลี่ยนรหัส) แล้ว
//   export ออกไปให้ middleware validateRequest(schema) ใน server.js เอาไปเช็คก่อนเข้า handler จริง
//   — ถ้าข้อมูลผิดรูปจะตอบ 400 ตั้งแต่ต้น ไม่หลุดไปถึง logic/DB
// จุดสำคัญ: schema เขียนให้ตรงกับ body จริงใน server.js (ไม่ใช่สเปกสมมติ) — ดู note ท้ายไฟล์
// ═══════════════════════════════════════════════════════════════════════════════════
