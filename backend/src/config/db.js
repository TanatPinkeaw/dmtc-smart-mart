// ═══════════════════════════════════════════════════════════════════════════════════
// 📄 config/db.js — ตัวเชื่อมต่อฐานข้อมูล MySQL (connection pool) + สร้าง/อัปเดตตารางตอนบูต
// ─────────────────────────────────────────────────────────────────────────────────────
// ทำอะไร: สร้าง mysql2 "pool" (คอนเนกชันหลายเส้นหมุนใช้ซ้ำ) แล้ว export pool ให้ทุกไฟล์ query ผ่าน;
//   ทุกคอนเนกชันตั้ง SET time_zone='+07:00' (เวลาไทย) เพื่อให้ NOW()/CURDATE() และคอลัมน์ TIMESTAMP
//   คืนค่าเป็นเวลาไทย ไม่ใช่ UTC — สำคัญมากต่อรายงาน/วันหมดอายุ (ดูคอมเมนต์ pool.on('connection'))
// จุดสำคัญ: initDB() รันตอนบูตครั้งแรก — สร้างตารางที่ยังไม่มี + ALTER เพิ่มคอลัมน์ใหม่แบบ idempotent
//   (รันซ้ำได้ไม่พัง); DB_SSL/DB_SSL_CA คุมการเข้ารหัส/verify cert ตอนต่อ DB บนคลาวด์ (เช่น Aiven)
// ═══════════════════════════════════════════════════════════════════════════════════
